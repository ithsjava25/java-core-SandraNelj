...
class WarehouseAnalyzer {
    ...
    public Map<Category, BigDecimal> calculateWeightedAveragePriceByCategory() {
        ...
        for (Map.Entry<Category, List<Product>> e : byCat.entrySet()) {
            ...
            for (Product p : items) {
                if (p instanceof Shippable s && s.weight() != null && s.weight().compareTo(BigDecimal.ZERO) > 0) {
                    double w = s.weight().doubleValue();
            ...
            BigDecimal avg;
            if (totalWeight > 0) {
                avg = weightedSum.divide(BigDecimal.valueOf(totalWeight), 2, RoundingMode.HALF_UP);
            } else {
    ...
    public List<ShippingGroup> optimizeShippingGroups(BigDecimal maxWeightPerGroup) {
        ...
        items.sort((a, b) -> Double.compare(
                ...
                a.weight() != null ? a.weight().doubleValue() : 0.0
        ));

        List<List<Shippable>> bins = new ArrayList<>();

        for (Shippable item : items) {
            double w = item.weight() != null ? item.weight().doubleValue() : 0.0;
            boolean placed = false;

            for (List<Shippable> bin : bins) {
                double binWeight = bin.stream()
                        .map(s -> s.weight() != null ? s.weight().doubleValue() : 0.0)
    ...
    public Map<Product, BigDecimal> calculateExpirationBasedDiscounts() {
        ...
        for (Product p : warehouse.getProducts()) {
            ...
            if (p instanceof Perishable per) {
                ...
                if (daysBetween == 0) {
                    discounted = discounted.multiply(new BigDecimal("0.50"));
                } else if (daysBetween == 1) {
                    discounted = discounted.multiply(new BigDecimal("0.70"));
                } else if (daysBetween > 1 && daysBetween <= 3) {
                    discounted = discounted.multiply(new BigDecimal("0.85"));
                } else {
                    discounted = p.price();
                }
        ...
        public InventoryValidation validateInventoryConstraints() {
            List<Product> items = new ArrayList<>(warehouse.getProducts());
        ...
        public InventoryStatistics getInventoryStatistics() {
            ...
            return new InventoryStatistics(
                    totalProducts, totalValue, averagePrice, expiredCount, categoryCount, mostExpensive, cheapest);
        }
